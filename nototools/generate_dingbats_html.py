...
from nototools import font_data, tool_utils, unicode_data
...
class SequenceList:
    ...
    def codes(self):
        ...
        codes |= {ord(cp) for cp in self.suffix}
        return codes
        ...
        for cp in self.codelist.codes():
            ...
            for rkey, keyinfos in self.used_fonts:
                ...
                for font, _, rcodelist in keyinfos:
                    ...
                    index += 1
                if cell_class:
                    line.append(f'<td class="{cell_class}">{cell_text}')
                else:
                    line.append("<td>&nbsp;")
...
def _read_target_data_from_file(filename):
    ...
    for line in strip_comments_from_file(filename):
        if line.startswith(kDefineDirective):
            ...
            name = name.strip()
            if name in defines:
                raise Exception(f"name {name} already defined in {filename}")
...
def _generate_fontkey(fonts, targets, data_dir):
    ...
    for tid, target in enumerate(targets):
        lines.append(f'<tr><th><a href="#target_{tid}">{target.name}</a>')
    ...
    for key, keyinfos in fonts:
        for font, name, _ in keyinfos:
            rel_font = path.relpath(font, data_dir) if font else "(no font)"
            lines.append(f"<tr><th>{key}<td>{name}<td>{rel_font}")
    lines.append("</table></div>")
    ...
    for key, keyinfos in fonts:
        ...
        for font, _, _ in keyinfos:
            ...
            if not font:
                ...
                classlines.append(
                    f'.{kname} {{ font-family: "{kname}", "noto_0" }}'
                )
...
def _parse_metrics_file(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            cy = int(cy)
            if cp in metrics:
                raise Exception(f"cp {cp:04x} listed twice in {filename}")
...
def _call_generate(
    ...
    if outfile:
        ...
        if not ext:
            ...
            raise Exception(
                f'mismatching format "{fmt}" and output extension "{ext}"'
            )
        outfile = base + "." + ext
