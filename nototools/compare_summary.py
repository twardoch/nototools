...
def print_difference(k, base_tuple, target_tuple, other_difference):
    ...
    if b_cmap != t_cmap:
        ...
        if added_in_target:
            print(
                "    cmap added: " + noto_lint.printable_unicode_range(added_in_target)
            )
...
def main():
    ...
    parser.add_argument(
        "--name", help="only examine files whose subpath+names contain this regex"
    )
    parser.add_argument(
        "--compare_size", help="include size in comparisons", action="store_true"
    )
    parser.add_argument(
