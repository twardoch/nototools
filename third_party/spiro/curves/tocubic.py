...
import struct
import sys
from math import atan2, cos, floor, hypot, log, pi, sin, sqrt
...
def bz_deriv(bz):
    degree = len(bz) - 1
    return [
        (degree * (bz[i + 1][0] - bz[i][0]), degree * (bz[i + 1][1] - bz[i][1]))
    ...
    if len(result) > 1 and optim > 2:
        ...
        for i in range(5):
            ...
            if max(score, tmid) < bscore:
                _bresult, bscore = result, max(score, tmid)
    return result
